...
export const FACTORY_ADDRESS = "0x9b924161F8006d359d50F292509eF730A5Ae9BB6"
...
export function fetchTokenSymbol(tokenAddress: Address): string {
  ...
  if (symbolResult.reverted) {
    ...
    if (!symbolResultBytes.reverted) {
      ...
      log.warning("Token symbol not defined in ERC20 Contract: {}", [
...
export function fetchTokenName(tokenAddress: Address): string {
  ...
  if (nameResult.reverted) {
    ...
    if (!nameResultBytes.reverted) {
      ...
      // Fallback to token list
      log.warning("Token name not defined in ERC20 Contract: {}", [
        tokenAddress.toHexString(),
      ])
...
export function fetchTokenDecimals(tokenAddress: Address): BigInt {
  ...
  if (!decimalResult.reverted) {
    ...
    log.warning("Token decimals not defined in ERC20 Contract: {}", [
      tokenAddress.toHexString(),
    ])
